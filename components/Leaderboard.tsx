  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        ...
        <p className="text-xs text-gray-400 mb-3">Every scoreline fans have predicted</p>
        {picks.length === 0 ? (
          <p className="text-sm text-gray-400 text-center py-4">
            No picks yet — predict some match scores in the Fixtures tab!
          </p>
        ) : (
          <div className="space-y-1.5 max-h-72 overflow-y-auto">
            {picks.map((p, i) => (
              ...
              >
                <span className="font-semibold text-gray-800 truncate max-w-[110px]">{p.name}</span>
                <span className="text-gray-400 text-xs">picks</span>
                <span className="font-bold text-gray-700 whitespace-nowrap ml-auto">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
        ...
        <p className="text-xs text-gray-400 mt-3">
          Predict any match in the Fixtures tab. Most points across the tournament wins!
        </p>
      </div>
    </div>
  );
